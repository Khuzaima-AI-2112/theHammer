...
You need three things from me, because two of them cannot be sent
automatically:

1. **The Portal address:**
   <https://thehammer-portal-282689937365.northamerica-northeast1.run.app/>
2. **Your invitation code** (the Portal cannot email these yet, so here it is):

   ```
   (sent to you privately — invitation codes are not kept in the repository)
   ```

   Single use, and it **expires on 8 September 2026**. It only works when you
   are signed in as chris.frosztega@gmail.com, so it is of no use to anyone
   else — but it is the key to the Workspace, so please do not forward it.
3. **The extension folder**, sent separately as a `.zip`. Unzip it somewhere
   permanent before installing — see section 2.
...
investigations were slowed by exactly that. A screenshot of the screen is
usually enough, and your screenshots have settled several questions already.

---

## 1. Getting into the Portal

1. Open <https://thehammer-portal-282689937365.northamerica-northeast1.run.app/>
2. Sign in with Google, using **chris.frosztega@gmail.com**.
3. Go to **Workspace Settings** in the left-hand menu.
4. In the **Join Workspace** box at the bottom, paste the code above and press
   **Join**.

**Working when:** the left-hand menu works and **Users** lists both of us.

**If it refuses the code**, there are three possible causes and they all produce
much the same message, so tell me which and I can tell them apart:

- You signed in with a different Google account.
- The code was mistyped — copy and paste it rather than retyping; a single
  wrong character looks identical to an expired code.
- It is past 8 September, or the code has already been used once.
...
3. Click **Load unpacked** and pick the extension folder I gave you.
4. **Put the folder somewhere permanent first.** Its location is part of the
   extension's identity, so moving it later breaks signing in (#36) and you
   would need me to change a setting on the server.
5. Pin The Hammer to the toolbar so you can reach it.

**Working when:** the card shows The Hammer with no red error, and the icon is
in your toolbar.

> ### ⚠️ Stop here and send me one thing
>
> On The Hammer's card at `chrome://extensions` there is an **ID** — 32 letters,
> something like `effbpmnbheohelnnpgpjdknofinhnagc`. **Copy it and send it to
> me before going further.**
>
> Chrome gives an extension loaded this way an identity derived from where its
> folder sits on your disk, so yours will differ from mine. The server only
> accepts Captures from identities it has been told about, and until I add
> yours it will silently refuse everything you send. Signing in will appear to
> work; the Captures will not arrive.
>
> Adding it takes me one change and one release — about ten minutes. **Wait for
> me to confirm before section 3**, and once you have the folder somewhere
> permanent, do not move it, or the ID changes and we do this again (#36).
...
> **The one thing to remember about updates.** When I send a new version of the
> extension, releasing it to the server does **not** update your browser.
> You must return to `chrome://extensions` and press the reload arrow ↻ on The
> Hammer's card. Reloading in place is safe; *moving* the folder is not. This
> caught us twice, and both times a fix looked broken when it simply had not
> loaded.
...
1. **Projects → New Project.** Name it `Chris test 1 Sept`.
...
This was broken until today and is the fault most likely to affect your
Storyboard work.
...
Until today it would have been empty, and any Capture taken would have been
recorded with no Tool at all — discovered only much later, in the export.
...
that person and confirm they cannot see `Chris test 1 Sept` — not in Projects,
...
- **Projects** — note the **Last capture** column always shows a dash. That is a
  known fault (#62), not a sign your Captures are missing. Use Activity instead.
