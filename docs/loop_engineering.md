...
*   **Benefit:** Prevents code drift and command trial-and-error by referencing static lookup registries ([`megamind.md`](../docs/megamind.md)) and historical gotchas ([`lessons_learned.md`](../lessons_learned.md)) before executing changes.
...
*   **Implementation:** Enforced via [`backend/src/lib/logger.js`](../backend/src/lib/logger.js) formatting.
...
*   **Activation Instructions:** Developers must run the installation script once to link the pre-commit hook into their local Git clones:
    ...
    ```
    This links the validation script [`verify-gcp-env.ps1`](../verify-gcp-env.ps1) and emulator test runner to the `.git/hooks/pre-commit` workflow.
...
*   **Implementation:** Enforced in `requireMultipart` checks inside [`backend/src/index.js`](../backend/src/index.js).
...
*   **Implementation:** Scoping document collections per test suite inside [`backend/tests/`](../backend/tests/).
...
*   **Activation Instructions:** Awaits trigger on the next Cloud Build deployment release cycle where the `inject-backend-url` build step in [`cloudbuild.yaml`](../cloudbuild.yaml) executes.
...
*   **Implementation:** Managed via service worker communications in [`extension/service-worker.js`](../extension/service-worker.js).
...
*   **Activation Instructions:** The basic dynamic config synchronization is active inside [`extension/popup.js#L291`](../extension/popup.js#L291) (`loadConfig()`). However, fully automating the Extension's API endpoint setup requires adding Webpack/Build token replacement variables to compile the extension popup and service worker dynamically rather than keeping hardcoded API URLs.
...
*   **Implementation:** Enforced via `validateWebhookUrl()` inside [`backend/src/index.js`](../backend/src/index.js#L173).
